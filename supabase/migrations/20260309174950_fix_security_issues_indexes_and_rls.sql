...
CREATE POLICY "Users can view own or account memberships"
  ...
  USING (
    ...
    OR account_id IN (
      SELECT account_id FROM account_members WHERE user_id::text = (select auth.uid())::text
    )
  );
...
CREATE POLICY "Users can view own time entries"
  ...
  USING (
    account_id::text IN (
      SELECT account_id::text FROM account_members WHERE user_id::text = (select auth.uid())::text
    )
  );
