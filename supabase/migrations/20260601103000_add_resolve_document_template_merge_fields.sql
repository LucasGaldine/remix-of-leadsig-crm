/*
  # Add centralized document template merge-field resolver

  This function provides a single backend source of truth for resolving
  dynamic variables used by document templates.
*/
...
BEGIN
  ...
  IF v_scope_of_work = '' THEN
    SELECT COALESCE(
      string_agg(format('%s. %s', scope_items.ordinality, btrim(scope_items.value)), E'\n' ORDER BY scope_items.ordinality),
      ''
    )
    INTO v_scope_of_work
    FROM jsonb_array_elements_text(COALESCE(v_lead.scope_of_work_items, '[]'::jsonb)) WITH ORDINALITY AS scope_items(value, ordinality)
    WHERE btrim(scope_items.value) <> '';
  END IF;
