/*
  # Format document template scope merge field as bullets

  Keep public.resolve_document_template_merge_fields(...) as the centralized
  source of truth for document variables while returning scope_of_work as a
  Markdown bulleted list. Currency merge fields remain formatted with leading
  dollar signs.
*/
...
BEGIN
  ...
  IF NULLIF(btrim(COALESCE(p_scope_of_work_override, '')), '') IS NOT NULL THEN
    SELECT COALESCE(
      string_agg(format('- %s', override_rows.line_text), E'\n' ORDER BY raw_override_rows.ordinality),
      ''
    )
    INTO v_scope_of_work
    FROM regexp_split_to_table(btrim(p_scope_of_work_override), E'\r?\n') WITH ORDINALITY AS raw_override_rows(raw_line, ordinality)
    CROSS JOIN LATERAL (
      SELECT regexp_replace(btrim(raw_override_rows.raw_line), '^([-*]|[0-9]+[.)])[[:space:]]+', '') AS line_text
    ) AS override_rows
    WHERE override_rows.line_text <> '';
  END IF;

  IF v_scope_of_work = '' THEN
    SELECT COALESCE(
      string_agg(format('- %s', checklist_rows.line_text), E'\n' ORDER BY checklist_rows.row_number),
      ''
    )
    INTO v_scope_of_work
    FROM (
      SELECT
        ...
        btrim(
          concat_ws(
            ': ',
            NULLIF(btrim(COALESCE(jci.label, '')), ''),
            NULLIF(btrim(COALESCE(jci.metadata ->> 'description', '')), '')
          )
        ) AS line_text
      FROM public.job_checklist_items jci
      WHERE jci.job_id = p_lead_id
    ) AS checklist_rows
    WHERE checklist_rows.line_text <> '';
  END IF;

  IF v_scope_of_work = '' AND v_estimate.id IS NOT NULL THEN
    SELECT COALESCE(
      string_agg(format('- %s', line_rows.line_text), E'\n' ORDER BY line_rows.row_number),
      ''
    )
    INTO v_scope_of_work
    FROM (
