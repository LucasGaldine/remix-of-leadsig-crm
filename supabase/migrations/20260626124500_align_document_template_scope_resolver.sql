/*
  # Align document template scope resolver with document previews

  Keep document merge fields centralized in the database while deriving
  scope_of_work from the same persisted sources used by previews:
  checklist item labels/descriptions first, then estimate line items.
*/
...
BEGIN
  ...
  IF v_scope_of_work = '' THEN
    SELECT COALESCE(
      string_agg(format('%s. %s', checklist_rows.row_number, checklist_rows.line_text), E'\n' ORDER BY checklist_rows.row_number),
      ''
    )
