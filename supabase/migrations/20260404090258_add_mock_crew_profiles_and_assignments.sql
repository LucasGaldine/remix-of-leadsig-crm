...
BEGIN
  ...
  IF _alert_enabled THEN
    ...
    VALUES (
      _target_account_id,
      _target_user_id,
      CASE WHEN TG_OP = 'INSERT' THEN 'New Job Assignment' ELSE 'Job Assignment Removed' END,
      CASE
        WHEN TG_OP = 'INSERT' THEN format('You have been assigned to job: %s', COALESCE(_lead_name, 'Unknown'))
        ELSE format('You have been removed from job: %s', COALESCE(_lead_name, 'Unknown'))
      END,
      CASE WHEN TG_OP = 'INSERT' THEN 'job_assigned' ELSE 'job_unassigned' END,
      _target_lead_id,
      'lead'
    );
  END IF;
