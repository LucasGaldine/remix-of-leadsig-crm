/*
  # Refine clock-in reminder audience rules

  Rules:
  - Single-member accounts: send reminders for all jobs with scheduled start times.
  - Multi-member accounts: send reminders only to users assigned to the job/schedule.
*/
...
BEGIN
  ...
  FOR _rec IN
    ...
    account_members_summary AS (
      SELECT
        am.account_id,
        COUNT(*) FILTER (WHERE am.is_active = true) AS active_member_count,
        MIN(am.user_id) FILTER (WHERE am.is_active = true) AS solo_user_id
    ...
    due_windows AS (
      SELECT
        ds.*,
        ams.active_member_count,
        ams.solo_user_id
      FROM due_schedules ds
      JOIN account_members_summary ams
        ON ams.account_id = ds.account_id
    ...
    reminder_recipients AS (
      SELECT DISTINCT
        ...
        dw.lead_id,
        dw.scheduled_date,
        dw.scheduled_time_start,
        dw.lead_name,
        dw.schedule_start_at,
        dw.solo_user_id AS user_id
      FROM due_windows dw
      WHERE dw.active_member_count = 1
        AND dw.solo_user_id IS NOT NULL
      ...
      JOIN public.job_assignments ja
        ON ja.lead_id = dw.lead_id
       AND ja.user_id IS NOT NULL
       AND (ja.job_schedule_id = dw.job_schedule_id OR ja.job_schedule_id IS NULL)
      WHERE dw.active_member_count > 1
    )
    SELECT rr.*
    ...
    WHERE NOT EXISTS (
      ...
      WHERE log.account_id = rr.account_id
        AND log.event_type = 'clock_in_reminders'
        AND log.status = 'sent'
        AND log.metadata ->> 'job_schedule_id' = rr.job_schedule_id::text
        AND log.metadata ->> 'user_id' = rr.user_id::text
    )
  LOOP
