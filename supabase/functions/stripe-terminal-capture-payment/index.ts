...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body: CapturePaymentRequest = await req.json();
    const { paymentIntentId, invoiceId, readerId, locationId } = body;
    ...
    const outcome = getTerminalCaptureOutcome(paymentIntentStatus);
    ...
    const paymentUpdate: Record<string, string | null> = {
      ...
      transaction_ref: latestCharge?.id ?? null,
      receipt_url: latestCharge?.receipt_url ?? null,
    };

    if (readerId) {
      paymentUpdate.stripe_terminal_reader_id = readerId;
    }

    if (locationId) {
      paymentUpdate.stripe_terminal_location_id = locationId;
    }

    if (outcome.paymentStatus) {
      paymentUpdate.status = outcome.paymentStatus;
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        paymentIntentStatus,
        paymentStatus: outcome.paymentStatus ?? paymentRecord.status,
    ...
    console.error("Error capturing Stripe Terminal payment:", error);
